...
public class Serveur extends Thread {
    int nb = 0; // Initialize an order counter.
    ...
    public void run() {
        try (ServerSocket ss = new ServerSocket(1234);) {
            while (true) {
                ...
                new ClientProcess(s, nb++).start(); // Create a new thread to handle the client and increment nomber client.
            }
        } catch (IOException e) {
    ...
    public class ClientProcess extends Thread {
        ...
        int nb; // Order in which the client connected.

        public ClientProcess(Socket s, int nb) {
            this.s = s;
            this.nb = nb;
        }

        public void run() {
            System.out.println("Client connected " + s.getRemoteSocketAddress() + " order: " + this.nb);
